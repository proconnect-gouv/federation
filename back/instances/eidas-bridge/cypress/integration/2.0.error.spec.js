...
describe('Error scenarios (EU Citizen / FR Service Provider)', () => {
  ...
  it('should return to FC+ oidc callback with an oidc error as the loa is lower than accepted', () => {
    ...
    cy.url().should(
      ...
      `${Cypress.env(
        'CORE_ROOT_URL',
      )}/api/v2/oidc-callback?error=eidas_node_error&error_description=StatusCode%3A%20Responder%0ASubStatusCode%3A%20undefined%0AStatusMessage%3A%20202019%20-%20Incorrect%20Level%20of%20Assurance%20in%20IdP%20response&state=`,
    );
  });
});

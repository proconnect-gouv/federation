...
export class FakeIdp {
  ...
  static create(
    ...
    issuer: string = IDP_ISSUER,
  ): FakeIdp {
    return new FakeIdp(idpFixture, issuer);
  }
  ...
  seed(app: INestApplicationContext): Promise<IdentityProvider> {
    return FakeIdentityProvider.create(this.idpFixture)
      .withFields({
        authzURL: `${this.issuer}/authorize`,
        discovery: false,
        id_token_signed_response_alg: "HS256",
        jwksURL: `${this.issuer}/jwks`,
        tokenURL: `${this.issuer}/token`,
        url: this.issuer,
        userinfo_signed_response_alg: "",
        userInfoURL: `${this.issuer}/userinfo`,
        ...this.fields,
      })
      .seed(app);
  }
  ...
  authenticate(
    ...
    // chain onto a prior authenticate() call's mock instead of overwriting it
    const fallback = jest.isMockFunction(oidcClient.fetch)
      ? (oidcClient.fetch as jest.Mock).getMockImplementation()
      : undefined;
    const issuer = this.issuer;

    jest
      .spyOn(oidcClient, "fetch")
      .mockImplementation(async (useHyyyperbridge, url) => {
        const target = String(url);
        if (!target.startsWith(issuer)) {
          if (fallback) return fallback(useHyyyperbridge, url);

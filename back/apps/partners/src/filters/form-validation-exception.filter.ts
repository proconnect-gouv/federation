    ...
    return payload.reduce(
      (acc, field) => {
        ...
        if (validators && validators.length > 0) {
          // create private method will not help understanding more
          const errorLabels = validators
            // eslint-disable-next-line max-nested-callbacks
            .filter((validator) => validator.errorLabel)
            // eslint-disable-next-line max-nested-callbacks
            .map((validator) => validator.errorLabel);

          acc[name] = errorLabels;
        }

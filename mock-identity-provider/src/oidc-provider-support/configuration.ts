...
import policy from "../oidc-provider-support/policy.ts";
import { findUserById } from "../user-data.ts";

export default function configuration(config: EnvConfig): Configuration {
  const {
    ServiceProviderAdapterEnv_CLIENT_ID: client_id,
    ServiceProviderAdapterEnv_CLIENT_SECRET: client_secret,
    ServiceProviderAdapterEnv_REDIRECT_URIS: redirect_uris,
    ServiceProviderAdapterEnv_POST_LOGOUT_REDIRECT_URIS:
      post_logout_redirect_uris,
    ServiceProviderAdapterEnv_SCOPE: scope,
    ServiceProviderAdapterEnv_ID_TOKEN_SIGNED_RESPONSE_ALG:
      id_token_signed_response_alg,
    ServiceProviderAdapterEnv_USERINFO_SIGNED_RESPONSE_ALG:
  ...
  return {
    ...
    clients: [
      {
        ...
        scope,
        id_token_signed_response_alg:
          id_token_signed_response_alg as SigningAlgorithmWithNone,
        userinfo_signed_response_alg:
          userinfo_signed_response_alg as SigningAlgorithmWithNone,
    ...
    findAccount: (_ctx: any, id: string) => {
      const user = findUserById(id);

      return {
        accountId: id,
        claims: () => user as AccountClaims,
      };
    },
    interactions: {

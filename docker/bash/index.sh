...
_command_register "inspect-files-for-app" "_get_app_files" "inspect-files-for-app <app> => Extract files needed for a built application, based on webpack stats. App must be built before running this command"
...
_command_register "ci-diff-files" "_ci_job_relevant_for_apps" "ci-diff-files <app1> [<app2> <app3> ...] => CI ONLY! Runs inspect-updated-files-for-apps but auto build the applications and fetches the git revision"

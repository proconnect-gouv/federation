...
function _ssl_generate() {
  if [ -f "${DKS_SSL_DIR}/app.crt" ] \
    && openssl x509 -checkend 86400 -noout -in "${DKS_SSL_DIR}/app.crt" &>/dev/null \
    && openssl x509 -noout -text -in "${DKS_SSL_DIR}/app.crt" | grep -q "${DKS_APP_DOMAIN}"; then
    return 0
  fi

...
import { linesToArray } from '@pc/shared/transforms/string.transform';
...
import { PaginationOptions, PaginationService } from '@pc/shared/pagination';
...
export class IdentityProviderService {
  ...
  async create(provider: IIdentityProviderDTO, user) {
    ...
    if (
      this.instanceService.isFcaLow() &&
      identityProviderInput.fqdns?.length > 0
    ) {
  ...
  async update(
    ...
    /**
     *  We need to update the corresponding fqdn for AgentConnect
     * note : we don't return the result of the update of the fqdns
     */
    if (this.instanceService.isFcaLow()) {
  ...
  async deleteIdentityProvider(id, user: string): Promise<DeleteResult> {
    ...
    if (this.instanceService.isFcaLow()) {
      await this.fqdnToProviderService.deleteFqdnsProvider(
  ...
  private tranformIntoLegacy(
    ...
    if (!this.instanceService.isFcaLow()) {
      legacyProvider.modal = {
        active: provider.modalActive,
        title: provider.modalTitle,
        body: provider.modalBody,
        continueText: provider.modalContinueText,
        moreInfoLabel: provider.modalMoreInfoLabel,
        moreInfoUrl: provider.modalMoreInfoUrl,
      };
    }

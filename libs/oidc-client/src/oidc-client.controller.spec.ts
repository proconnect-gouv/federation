...
describe('OidcClient Controller', () => {
  ...
  beforeEach(async () => {
    ...
    oidcClientController = module.get<OidcClientController>(OidcClientController);
  ...
  describe('redirectToIdp', () => {
    it('Should call oidc-client-service for retrieve authorize url', async () => {
      ...
      const mockedoidcClientService = 'https://my-authentication-openid-url.com';

      oidcClientServiceMock.getAuthorizeUrl.mockReturnValueOnce(mockedoidcClientService);
  ...
  describe('getOidcCallback', () => {
    it('Should call oidc-client-service for retrieve authorize url', async () => {
      ...
      expect(oidcClientServiceMock.getUserInfo).toHaveBeenCalledWith(accessToken);
      expect(req.session.user).toEqual(userinfo);
      expect(req.session.idToken).toEqual(idToken);
      expect(res.redirect).toHaveBeenCalledTimes(1);
    });
  });
});
